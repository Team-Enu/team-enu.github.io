...
**** ~link_map~ 構造体
   ...
   gdbから、これらの構造体の定義を得るために、libcにおけるデバッグ情報を取得しておく。
   #+begin_src
mc4nf@mc4nf:~/ctf/seccon2021/pwn/kasu_bof$ sudo apt-get install libc6-dbg libc6-dbg:i386
   #+end_src
   
   それでは、構造を知るために、gdbを用いて定義を参照してみる。
   #+begin_src
gef➤  break main
gef➤  run
...
...
   ...
   - https://elixir.bootlin.com/glibc/glibc-2.36/source/include/link.h#L95
     ...
     *return to dl resolve* において、 ~link_map~ 構造体の ~offset~ が32の位置に保持されている ~l_info~ フィールドを利用する。
      ~l_info~ は、後述する ~.dynamic~ セクションの情報を含んでおり、各セクションへのアドレスを保持している。

     各セクションが格納されている ~l_info~ のインデックスは、 ~d_tag~ に格納される。
      - d_tagに代入されうる値の一覧
	  ...
	  ...
	  #+end_src
     例えば、 シンボルの文字列が格納されている ~.dynstr~ セクションのアドレスを参照したい場合、
     ~l_info[DT_STRTAB]~ とすることで参照することができる。

     
   それでは、実際にgdbでプログラムを動作させ、 ~link_map~ 構造体の中身を参照してみる。
   
   ここで、 ~_dl_runtime_resolve~ の第一引数である ~0xf7ffd990~ は ~link_map~ 構造体のアドレスであるため、 ~0xf7ffd990~ を ~link_map~ 構造体のアドレスとし、表示してみる。
   #+begin_src 
...
$1 = {
  ...
  l_real = 0xf7ffd990,
  l_ns = 0x0,
  l_libname = 0xf7ffdc78,
  l_info = {0x0, 0x804bf14, 0x804bf84, 0x804bf7c, 0x0, 0x804bf54, 0x804bf5c, 0x0, 0x0, 0x0, 0x804bf64, 0x804bf6c, 0x804bf1c, 0x804bf24, 0x0, 0x0, 0x0, 0x804bf9c, 0x804bfa4, 0x804bfac, 0x804bf8c, 0x804bf74, 0x0, 0x804bf94, 0x0, 0x804bf2c, 0x804bf3c, 0x804bf34, 0x804bf44, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x804bfbc, 0x804bfb4, 0x0 <repeats 13 times>, 0x804bfc4, 0x0 <repeats 25 times>, 0x804bf4c},
  l_phdr = 0x8048034,
...
   #+end_src

   すると ~link_map~ 構造体のそれぞれのメンバの値が確認できる。
   例えば、 ~.dynstr~ セクションの ~d_tag~ は ~5~ なため、 ~l_info[5]~ の ~0x804bf54~ が  ~.dynstr~ セクションのアドレスということがわかる。
   
   それでは、次にライブラリ関数のシンボルを解決する際に必要な情報が格納されている ~.dynamic~ セクションを見ていく。
   
**** ~.dynamic~ セクション
  ~.dynamic~ セクションは、 ~ELF_Dyn~ 構造体のエントリを ~d_tag~ の種類の数保持しており、このセクションの中に ~.rel.plt~ や ~.dynsym~ , ~.dynstr~ などのセクションのアドレスが格納されている。

  それでは、ELF32_Dyn 構造体をgdbで参照してみる。
  	#+begin_src
...
/*                 4 */        Elf32_Addr d_ptr;
  ...
  # そこで、 ~.dynamic~ セクションを参照することで、これらのアドレスの情報を取得していく。

  
  ~.dynamic~ セクションそのもののアドレスは、 ~link_map~ 構造体の ~l_info[DT_JMPREL]~ に格納されている。ここで、 ~DT_JMPREL~ は23のため、 ~l_info[23]~ を参照する。
  #+begin_src
gef➤  set $dynamic = $link_map.l_info[23]
gef➤  print *$dynamic
$3 = {
  d_tag = 0x17,
  d_un = {
  ...
  #+end_src
  上記の結果より、 ~.dynamic~ セクションのアドレスは ~0x804bf14~ であるとわかる。  
