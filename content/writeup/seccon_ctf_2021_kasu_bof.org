...
**** ~link_map~ 構造体
    ...
    *return to dl resolve* において、後述する ~.dynamic~ セクションの情報を含む ~l_info~ 構造体を利用する。
   ...
   また、実際にgdbでプログラムを動作させ、 ~link_map~ 構造体に格納されている値を参照してみる。
   このため、以下のサイトを参考に、gdbで、ELFの構造体を読み込めるようにする。
   ...
   それでは早速、 ~_dl_runtime_resolve~ の第一引数である ~0xf7ffd990~ は ~link_map~ 構造体のアドレスであるため、 ~0xf7ffd990~ に配置されている ~link_map~ 構造体を表示してみる。
   #+begin_src 
   ...
   #+end_src
   # l_infoを含む構造体を定義できないため苦戦中

   実際にそれぞれのメンバに値が格納されていることを確認できた。
   
   それでは、次にライブラリ関数のシンボルを解決する際に必要な情報が格納されている ~.dynamic~ セクションを見ていく。
   
**** ~.dynamic~ セクション
  ~.dynamic~ セクションは、 ~ELF_Dyn~ 構造体のエントリを ~d_tag~ の種類の数保持しており、このセクションの中に ~.rel.plt~ や ~.dynsym~ , ~.dynstr~ などのアドレスが格納されている。

  そこで、 ~.dynamic~ セクションを参照することで、これらのアドレスの情報を取得していく。

  
  ~.dynamic~ セクションそのもののアドレスは、 ~link_map~ 構造体の ~l_info[DT_JMPREL]~ に格納される。
